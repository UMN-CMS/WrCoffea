...
if len(sys.argv) < 6:
    print("Usage: python plot_expected_limits_filtered.py <combine_output_2jet.root> <combine_output_3jet.root> <theory_xsec.txt> <output_name.pdf> <MW_GeV>")
    sys.exit(1)

combine_file = sys.argv[1]
combine_file_3jet = sys.argv[2]
theory_file = sys.argv[3]
outfile = sys.argv[4]
target_mW = float(sys.argv[5])   # e.g. 100
    ...
    for line in f:
        if line.strip() and not line.startswith('#'):
            ...
            if len(parts) >= 3:
                ...
                #target_mN = MW/2
                if abs(MW - target_mW) < 1e-3:     # match this N mass
                    #print(MW, MN, X, target_mW)
                    masses.append(MN)
                    xsecs.append(X)
                    #xsec_unc.append(dX)
...
has_mN = any("W" in b for b in branches)
branches_3jet = [b.GetName() for b in tree_3jet.GetListOfBranches()]
has_mN_3jet = any("W" in b for b in branches_3jet)
...
for event in tree:
    ...
    mN = int(event.mh% 10000) #getattr(event, "mN", target_mW) if has_mN else target_mW
    ...
    if abs(mWR - target_mW) > 1e-3:
        continue
    #print(mWR, mN, target_mW, "printing particle masses")
    ...
    if (mN not in mass_vals) : #or (mWR!=6001) or (mWR !=8001):
        print(mN)
        mass_vals.append(mN)
        #print(q, val, mWR, mN)
...
print(mass_vals)
if not exp_median:
    print(f"[ERROR] No expected limits found for MN = {target_mW} GeV.")
    sys.exit(1)
...
for event in tree_3jet:
    ...
    if (mN not in mass_vals_3jet) : #or (mWR!=6001) or (mWR !=8001):
        print(mN)
        mass_vals_3jet.append(mN)
        #print(q, val, mWR, mN)
mass_vals_3jet = sorted(list(set(mass_vals_3jet)))
print(mass_vals_3jet)
...
gr_median_3jet.SetLineColor(kBlue+2)
...
leg.AddEntry(gr_median_3jet, "Expected limit (Single Bin)", "l")#"Expected limit (3-jet)", "l")
# leg.AddEntry(gr_68_3jet, "68% expected (3-jet)", "f")
# leg.AddEntry(gr_95_3jet, "95% expected (3-
...
desc.DrawLatex(0.15, 0.80, f"Mw = {target_mW}")
