...
# Configuration
# --------------------------
Year = "RunII/2018/RunIISummer20UL18/alljets"
channels = ["EE", "MuMu"]
regions = ["Resolved"]
...
for channel in channels:
    for region in regions:
        hist_type = "best_fiveobject" if region == "Resolved" else "mass_twoobject"
        dir_name = f"wr_{channel.lower()}_{region.lower()}_sr"
        
        for mass in masses:
            card_file = f"{outdir}/WR_3jets_{channel}_{region}_{mass}.txt"
            ...
            with open(card_file, "w") as f:
                ...
                for group, samples in groups.items():
                    ...
                    for sample in samples:
                        sample_file = f"WR_Plotter/rootfiles/{Year}/WRAnalyzer_{mass[7:]}_{sample}.root"
                        #print(sample_file)
                        if not os.path.exists(sample_file):
                            print('[Warning] File name that is not found :', sample_file)
                            continue
                        fin = ROOT.TFile.Open(sample_file)
                        h = fin.Get(f"{dir_name}/{hist_type}_{dir_name}")
                        if not h:
                    ...
                    # if (i >=1):
                    tmpfile = f"WR_Plotter/rootfiles/{Year}/WRAnalyzer_{mass[7:]}_{group}.root"
                    #     fout = ROOT.TFile(tmpfile, "RECREATE")
                    #     hist_sum.Write(group)
                    #     fout.Close()
                    f.write(f"shapes {group} * {tmpfile} {dir_name}/{hist_type}_{dir_name} \n")
