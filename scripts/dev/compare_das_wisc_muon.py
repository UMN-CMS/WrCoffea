#!/usr/bin/env python3
"""
compare_das_wisc_muon.py

Compares DAS file lists for each Muon Run2024 dataset
...
MUON_MAP = {
    ...
    "Muon0_Run2024I": "/Muon0/Run2024I-MINIv6NANOv15_v2-v1/NANOAOD",
    "Muon1_Run2024I": "/Muon1/Run2024I-MINIv6NANOv15_v2-v1/NANOAOD",
}
...
def get_wisc_files(muon_dir: str) -> list[str]:
    """Return list of skim file basenames from Wisconsin Tier-2."""
    full_dir = os.path.join(BASE_DIR.rstrip("/"), muon_dir)
...
def compare(dataset: str, muon_dir: str) -> None:
    """Compare DAS and Wisconsin files for one Muon dataset."""
    print("=" * 90)
    print(f"Dataset    : {dataset}")
    print(f"Wisc folder: {muon_dir}")

    das_files = get_das_files(dataset)
    wisc_files = get_wisc_files(muon_dir)
...
# -----------------------------------------------------------------------------
def main():
    print("Comparing Muon datasets between DAS and Wisconsin storage...\n")
    for muon_dir, dataset in MUON_MAP.items():
        compare(dataset, muon_dir)
    print("\nDone.\n")
